...
$(window).on("load", () => {
  ...
  var confettiWidth = function () {
    return VX.rfb(confettiWidthMin, confettiWidthMax);
  };
  var confettiInitX = function () {
    return VX.rfb(0, VX.interfaceWidth);
  }; // initial confetti top left corner x position
  var confettiInitY = function () {
    return VX.rfb(-confettiWidthMax, VX.interfaceHeight);
  }; // initial confetti top left corner y position
  ...
  var tropicalSunsetPalette = [
    `rgba( 254,218,132, ${confettiAlpha} )`,
  ...
  function Confetti(initX, initY) {
    ...
    this.sk = VX.addSkin([this.pt1, this.pt2, this.pt3, this.pt4], {
  ...
  function runInitialBurst() {
    for (var i = 0; i < confettiVolumeInit; i++) {
      ...
      var initConfettiX =
        VX.interfaceWidth * 0.5 +
        Math.cos(initAngle - Math.PI / 2) * initDistance;
      var initConfettiY =
        VX.interfaceHeight * 0.6 +
        Math.sin(initAngle - Math.PI / 2) * initDistance;
      ...
      confettis[confettis.length - 1].pt1.px =
        initConfettiX + Math.cos(velocityAngle - Math.PI / 2) * velocity;
      confettis[confettis.length - 1].pt1.py =
        initConfettiY +
        Math.sin(velocityAngle - Math.PI / 2) * velocity +
        VX.interfaceWidth * 0.01;
    }
  }
  ...
  function runInitialFall() {
    ...
    confettiInitY = function () {
      return -confettiWidthMax * 3;
    };
  }
  ...
  function moveConfettisOnMouseOver() {
    for (var i = 0; i < confettis.length; i++) {
      ...
      if (
        xDiff <= mouseZoneRadius &&
        yDiff <= mouseZoneRadius &&
        mouseCoords.px != null &&
        mouseCoords.py != null
      ) {
  ...
  function popperBurst(event) {
    ...
    for (var i = 0; i < confettis.length; i++) {
      ...
      var intensity =
        (popperBurstZoneRadius - dist) * popperBurstIntensity * 0.0005;
    ...
    //new confettis burst
    var volume =
      confettis.length < confettiVolumeMax - popperBurstVolumeMax
        ? popperBurstVolumeMax
        : confettiVolumeMax - confettis.length;
    for (var i = 0; i < volume; i++) {
      ...
      confettis[confettis.length - 1].pt1.px =
        mouseCoords.cx + Math.cos(angle - Math.PI / 2) * velocity;
      confettis[confettis.length - 1].pt1.py =
        mouseCoords.cy + Math.sin(angle - Math.PI / 2) * velocity;
    }
  }
